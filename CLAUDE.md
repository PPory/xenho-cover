...
- 输入一篇文章或一个主题，产出一份可直接贴到即梦 / Midjourney / nano banana 的完整图像生成提示词
...
跑一遍端到端：拿一篇文章触发 skill → 应先问平台 → 选定后检查产出提示词：无 `{{占位符}}` 残留、无 STYLE.md 原文整段拼贴、标题层落在风格语言里。最终以即梦实际出图对照 `refs/` 校准。
